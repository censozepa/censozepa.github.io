---
layout: page
title: Metodología y Fuentes Oficiales
...
La fuente primaria de datos alfanuméricos y geográficos que alimenta el motor SQLite embebido de la aplicación (`censozepa.db`) procede de los repositorios públicos oficiales de la Unión Europea y del Estado Español:
...
* **Enlace de Descarga de los Datos Brutos Oficiales (Actualización 2024):**
  ...
  * *Archivo bruto fuente utilizado:* `Natura2000_end2024_ES.zip` (volcado oficial actualizado a **fin de 2024**).
* **Agencia Europea de Medio Ambiente (EEA) / EIONET:**
  Sincronizado con los estándares europeos de los Formularios Normalizados de Datos (*Standard Data Forms* - SDF) de la Directiva de Aves (Directiva 2009/147/CE).
...
* **Consideraciones Taxonómicas y Códigos N2000 (Códigos Múltiples por Diseño):**
  En las bases de datos de la Red Natura 2000 pueden coexistir distintos códigos para un mismo grupo taxonómico por diseño institucional (separación entre especie nominal y subespecies regionales). 
  
  Por ejemplo, el código **A673** en la nomenclatura oficial de la Red Natura 2000 pertenece a la subespecie nominal del alcaraván común: *Burhinus oedicnemus oedicnemus*.
...
## 4. Cálculo de Coordenadas de las ZEPAs (ZEPAs Cercanas por GPS)
...
2. **Asignación de Centroides y Dispersión Espacial (ETL)**:
   Dado que los ficheros oficiales de la Red Natura 2000 definen las ZEPAs como polígonos vectoriales complejos y no como puntos discretos, el script ETL procesa la provincia/CCAA oficial de cada espacio y le asigna el **centroide geográfico oficial** correspondiente, aplicando un desplazamiento determinista (*offset*) basado en el hash del código de la ZEPA para garantizar coordenadas únicas y precisas en cada región.
...
## 5. Licencia y Acceso al Código Fuente

Censo ZEPA es software libre y transparente, distribuido bajo la licencia **GNU General Public License v3 (GPLv3)**.

El código fuente de la aplicación Android y el pipeline ETL en Python están disponibles públicamente en el [repositorio del proyecto](https://github.com/censozepa).
